...
type UserRoleService interface {
	...
	UpdateUserRole(request user_role.UpdateUserRoleRequest) (user_role.UserRoleResponse, error)
...
func (s *userRoleService) UpdateUserRole(request user_role.UpdateUserRoleRequest) (user_role.UserRoleResponse, error) {
	role, err := s.roleRepository.Role(request.RoleID)
	if err != nil || role.ID == 0 {
		return user_role.UserRoleResponse{}, errors.New("role not found")
	}
	...
	userRole, err := s.userRoleRepository.Update(data)
	if err != nil {
		return user_role.UserRoleResponse{}, errors.New("something wrong")
	}
